// GameBattleRPG.cpp : This file contains the 'main' function. Program execution
...
Character *creatCharacter(string userName, int userJob) {
  ...
  hero->printInfo();
  cout << "Name: " << hero->getName() << ", Job: " << hero->getJob()
       << " created." << endl;
  return hero;
}
...
Character *loadCharacter() {
  ...
  if (loadFile.is_open()) {
    ...
    ss >> charName >> charJob >> charLevel >> charEXP >> charAtt >> charDef >>
        charHP >> charmaxHP;

    if (charJob == "Knight") {
      loadedChar = new Knight(charName, charJob, charLevel, charEXP, charAtt,
                              charDef, charHP, charmaxHP);
    } else if (charJob == "Wizard") {
      loadedChar = new Wizard(charName, charJob, charLevel, charEXP, charAtt,
                              charDef, charHP, charmaxHP);
    } else if (charJob == "Babarian") {
      loadedChar = new Babarian(charName, charJob, charLevel, charEXP, charAtt,
                                charDef, charHP, charmaxHP);
    } else if (charJob == "Rogue") {
      loadedChar = new Rogue(charName, charJob, charLevel, charEXP, charAtt,
                             charDef, charHP, charmaxHP);
    } else if (charJob == "DemonKing") {
      loadedChar = new DemonKing(charName, charJob, charLevel, charEXP, charAtt,
                                 charDef, charHP, charmaxHP);
    } else {
      cout << "Invalid!!!" << endl;
      return nullptr;
...
void saveGame (Character* characterSave){
  ofstream saveFile("character.txt"); 
  saveFile << characterSave->getName() << " " << characterSave->getJob() << " "
           << characterSave->getLevel() << " " << characterSave->getExp() << " "
           << characterSave->getAttack() << " " << characterSave->getDef() << " "
           << characterSave->getHP() << " " << characterSave->getmaxHP() << " "
           << characterSave->getSkill() << " " << characterSave->getDamage();
  saveFile.close();
}
...
int main() {
  ...
  while (true) {
    ...
    while (true) {
      while (true) {
        cout << "FINDING GOBLIN !!! PLEASE WAIT !!!" << endl;
        cout << endl;
        ...
        if (choice == 2) {
          ...
          while (true) {
            cout << yourChar->getName() << " use " << yourChar->getSkill()
                 << " deal " << yourChar->getDamage() << " to the Goblin" << endl;
            enemy.setHP(enemy.getHP() - yourChar->getDamage() -
                        yourChar->getAttack());
            if (enemy.getHP() <= 0) {
              enemy.setHP(0);
            }
            ...
            if (enemy.getHP() <= 0) {
              ...
              if (yourChar->getExp() >= 100) {
                yourChar->levelUp();
                cout << "################# LEVEL UP ################" << endl;
                cout << "----- " << yourChar->getName() << " is now LEVEL "
                     << yourChar->getLevel() << " -----" << endl;
              ...
              string usePotion;
              cout << "Enemy attacked deal " << enemy.getAttack() << " damage."
                   << endl;
              yourChar->setHP(yourChar->getHP() + yourChar->getDef() -
                              enemy.getAttack());
              if (yourChar->getHP() > yourChar->getmaxHP()) {
                yourChar->setHP(yourChar->getmaxHP());
              }
              cout << "Your HP: " << yourChar->getHP() << " / "
                   << yourChar->getmaxHP() << endl;
              ...
              // Using potion
              cout << "Do you want to use potions? Enter \"yes\" to use, "
                      "\"no\" to continue the battle."
                   << endl;
              cin >> usePotion;
              cout << endl;
              if (usePotion == "yes") {
                ...
                cout << "============================================" << endl;
                int healedChoice;
                cout << "Do you want to continue attack or flee to next enemy? "
                        "1 Attack 2 Flee"
                     << endl;
                cin >> healedChoice;
                cout << "============================================" << endl;
      ...
      int choice;
      cout << "Do you want to take rest and continue? 1 Rest & Continue 2 Save "
              "& Exit"
           << endl;
      cin >> choice;
      cout << endl;
